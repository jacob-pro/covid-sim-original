...
{ if (avi==NULL) return AVIERR_BADHANDLE;
  ...
  { AVICOMPRESSOPTIONS myopts; ZeroMemory(&myopts,sizeof(myopts));
    ...
    hr = AVIStreamSetFormat(au->psCompressed, 0, &dibs.dsBmih, (long) (dibs.dsBmih.biSize+dibs.dsBmih.biClrUsed*sizeof(RGBQUAD)));
    if (hr!=AVIERR_OK) {au->iserr=true; return hr;}
  }
  //
  return AVIERR_OK;
...
HRESULT AddAviFrame(HAVI avi, HBITMAP hbm,unsigned char *pl)
{ 
  static int i,tmp[10000];
  unsigned char *palette;
  BITMAPINFOHEADER *dibs2;

  if (avi==NULL) return AVIERR_BADHANDLE;
  if (hbm==NULL) return AVIERR_BADPARAM;
  DIBSECTION dibs; int sbm = GetObject(hbm,sizeof(dibs),&dibs);
  dibs2=(BITMAPINFOHEADER *) tmp;
  *dibs2=dibs.dsBmih;
  palette=(unsigned char *)tmp + dibs.dsBmih.biSize/sizeof(unsigned char);
  for(i=0;i<1024;i++) palette[i]=pl[i];
  if (sbm!=sizeof(DIBSECTION)) return AVIERR_BADPARAM;
  TAviUtil *au = (TAviUtil*)avi;
  if (au->iserr) return AVIERR_ERROR;
  //
  ...
  { AVICOMPRESSOPTIONS opts; ZeroMemory(&opts,sizeof(opts));
    ...
    if (hr != AVIERR_OK) {au->iserr=true; return hr;}
	hr = AVIStreamSetFormat(au->psCompressed, 0, dibs2, (long) (dibs.dsBmih.biSize+dibs.dsBmih.biClrUsed*sizeof(RGBQUAD)));
	if (hr!=AVIERR_OK) {au->iserr=true; return hr;}
}
  //
